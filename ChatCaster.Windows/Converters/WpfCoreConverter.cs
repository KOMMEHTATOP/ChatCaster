...
namespace ChatCaster.Windows.Converters
{
    ...
    public static class WpfCoreConverter
    {
        ...
        private readonly static Dictionary<WpfKey, CoreKey> KeyMap = new()
        {
            ...
            // Дополнительные Numpad клавиши из Core
            ...
            // Note: NumPadEnter нет в стандартном WPF Key enum
        ...
        public static KeyboardShortcut? CreateKeyboardShortcut(WpfKey wpfKey, WpfModifierKeys wpfModifiers)
        {
            ...
            if (coreKey == null)
            {
                System.Diagnostics.Debug.WriteLine($"[WpfCoreConverter] Unsupported key: {wpfKey}");
                return null;
            }
            ...
            System.Diagnostics.Debug.WriteLine($"[WpfCoreConverter] Created shortcut: {wpfKey}+{wpfModifiers} -> {shortcut.DisplayText}");
            
            return shortcut;
        }

        /// <summary>
        /// Проверяет, поддерживается ли конверсия для данной WPF клавиши
        /// </summary>
        /// <param name="wpfKey">WPF клавиша</param>
        /// <returns>true если клавиша поддерживается</returns>
        public static bool IsKeySupported(WpfKey wpfKey)
        {
            return KeyMap.ContainsKey(wpfKey);
        }

        /// <summary>
        /// Получает список всех поддерживаемых WPF клавиш
        /// </summary>
        /// <returns>Коллекция поддерживаемых клавиш</returns>
        public static IReadOnlyCollection<WpfKey> GetSupportedKeys()
        {
            return KeyMap.Keys;
        }
